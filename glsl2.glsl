...
//    pMirrorOctant(p.xz, vec2(2.0,4.0));
    pMod1(p.z, separation);
    pR(p.xy, 80.0);
    float columnHeight = 4.0;
    float column;
    float d;
    float colR = 0.4;
    vec3 boxSize = vec3(1.0, 0.3, 1.0);

    //Columm
    float colBox1 =  fBox(p, vec3(0.4, columnHeight, 0.4));
    pR45(p.xz);
    float colBox2 =  fBox(p, vec3(0.4, columnHeight, 0.4));
    float colBox = fOpUnionStairs(colBox1, colBox2, 0.2, 2.5);
    pR(p.xz, -45.0);

    p.y += columnHeight;
    float b1 = fBox(p, boxSize);
    column = fOpUnionColumns(colBox, b1, colR, 3.0);
    p.y -= columnHeight * 2.0;
    float b2 = fBox(p, boxSize);
    column = fOpUnionColumns(column, b2, colR, 5.0);
    p.y += columnHeight;

//    //Stair
//    vec3 stairSize = vec3(separation , separation / 2.0 , 3.0);
////    p.y += columnHeight;
//    float stairBox = fBox(p, stairSize);
//    pR(p.xy, -25.0);
//    p.x += stairSize.x / 2.0;
//    stairSize.x *=2.0;
//    float stairBox2 = fBox(p, stairSize);
//    float stairs = fOpIntersectionStairs(stairBox, stairBox2, 2.2, 20.0);
//    pR(p.xy, 25.0);
//    p.x -= stairSize.x / 2.0;
//    p.y -= stairSize.y;
//    stairs = max(stairs,fBox(p, stairSize));
//    return stairs;
    return column;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 ro = 0.6*vec3(2.0,-15.0, 40.0);
	vec3 ta = 0.5*vec3(0.0, 40.0,-4.0);

    ro.x-=cos(iGlobalTime / 10.) * 2.;
